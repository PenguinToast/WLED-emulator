...
- The kick detector tracks a blended sub/punch energy envelope, current song baseline, current peak, `samplePeak` hints, and recent kick spacing so the main pulse adapts across different EDM mixes instead of relying on one fixed bass threshold.
