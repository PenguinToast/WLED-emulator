...
With the ESP32-S3-friendly onset tracker and current live-control defaults, the 30s house reference estimates a 128.6 BPM grid with high confidence and the visual pulses match 61 of 65 beats. Across the broader mixed test set, low tail duty remains the main safety check; low-confidence tempo grids on dubstep references are useful for smoke testing response, but not as authoritative beat labels.
