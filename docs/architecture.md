...
8. `tools/run-cpp-effect.mjs` can compile and run the C++ harness, then stream RGB frames back into `/api/emulator/frame`.
...
- `src/server/wled-catalog.mjs`: parser for vendored WLED `FX.cpp` and `FX.h` effect metadata.
