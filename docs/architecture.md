...
Native frame transport uses compact flat RGB hex strings end to end. The C++ process emits that format, the runner forwards it over the frame WebSocket, the server broadcasts it without expanding it, and `src/emulator/native-frame.ts` decodes it into the browser LED model just before painting.
